...
指数E=10000101  实际指数为6，因为组合的数可以写成1.111011101*2^6，又因为E用的是偏移量计算的，
    			因此6+127=133，转化为2进制就是10000101
尾数M=1110 1110 1000 0000 0000 000   
组合的数科学计数法表示1.111011101*2^6，即去掉1，所以尾数为111011101,后面补0直到23位。
...
![6.1_1](image/.C&C++_image/6.1_1.png)

![6.1_2](image/.C&C++_image/6.1_2.png)

![6.1_3](image/.C&C++_image/6.1_3.png)

**优先级记忆**：

括号（**()**、**[ ]**）、选择（**.** 、**->**）> 单目、**`sizeof`** >算术（乘除模>加减）>左移、右移>关系（等于、不等于优先级同低）>位运算符（按位与>按位异或>按位或）>逻辑（与>或）>条件>赋值>逗号
...
自动转换由编译系统自动完成（整数默认为`int`，小数默认为`double`）

此处采用标准为`C99`

自动转换遵循以下规则：

1. 参与运算的量类型不同，需要先转换为同一类型，然后进行运算。

2. 转换规则为

	![6.2.1_1](image/.C&C++_image/6.2.1_1.png)

	说明：

	除了红色箭头的是必定发生的，其余蓝色箭头是可以跳跃的，表达式只要有蓝色箭头类型中靠右的一个类型，选择最高类型，从`int`开始到最高类型的前一个类型全体转换为最高类型。

	**详细说明**：

	1. `bool`型、`char`型和`short`型在运算时会被转换为`int`型。【有则必定发生】
	2. 如果运算中涉及到`unsigned int`型，那么`int`型会被转换为`unsigned int`型。
	3. 如果运算中涉及到`long`型，那么`int`型和`unsigned int`型会被转换为`long`型。
	4. 如果运算中涉及到`unsigned long`型，那么`long`型会被转换为`unsigned long`型。
	5. 如果运算中涉及到`float`型，那么`int`型、`long`型、`unsigned int`型和`unsigned long`型会被转换为`float`型。
	6. 如果运算中涉及到`double`型，那么`float`型、`int`型、`long`型、`unsigned int`型和`unsigned long`型会被转换为`double`型。
...
1. **整型提升**：首先，任何小于int的整型（如`char`、`short`）会被提升为`int`或`unsigned int`。这是在**运算之前**进行的，以确保所有的整型操作至少以int的精度进行。
2. **算术转换**：在整型提升之后，如果表达式中仍然存在不同类型的数值，那么会进行算术转换，以确保运算的两边类型一致。



因此表达式中的类型转换是在运算过程中逐步进行的，而不是在运算开始之前一次性完成所有转换，比如：当`char`型和`float`型参与运算时，`char`型会首先被转换为int型，然后`int`型再被转换为`float`型，最后进行运算，因此不是一步到位。
...
- **静态类型转换**是`C++`中一种常用的强制类型转换方法，适用于基本数据类型之间的转换。它在编译时进行类型检查，不涉及运行时的类型检查，因此效率较高。
...
- **动态类型转换**主要用于处理多态类层次结构中的类型转换。它在运行时进行类型检查，确保转换的安全性。
...
- `const_cast`用于移除或添加`const`属性，不改变变量的类型
...
- `reinterpret_cast`用于低级别的类型转换，可以将任何指针类型转换为任何其他指针类型，或者将指针类型转换为足够大的整型。
